...
const RestaurantCategory = ({cardData, showMenuItems, setShowIndex}) => {
    ...
    if(cardData !== undefined) {
        ...
        <div className="w-full">
            <div className="w-6/12 mx-auto my-4 bg-gray-50 shadow-lg">
                <div className="p-4 flex justify-between cursor-pointer" onClick={() => setShowIndex()}>
                    <span className="font-bold text-lg">{cardData?.title} ({cardData?.itemCards?.length})</span>
                    <span>{showCategoryItems ? '⬆️' : '⬇️'}</span>
                </div>
                <div className="p-4">
                    {
                        showMenuItems ? (<CategoryItemList items={cardData?.itemCards} />) : ''
                    }
                </div>
            </div>
